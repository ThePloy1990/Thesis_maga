...
celery.conf.beat_schedule = {
    'collect-stock-data-daily': {
        'task': 'src.celery_app.collect_stock_data_task',
        'schedule': crontab(hour=0, minute=0),  # Каждый день в полночь
        'args': (['^SPY', 'AAPL', 'MSFT', 'GOOGL', 'AMZN'], '1d', '1d'),
    },
    'collect-crypto-data-hourly': {
        'task': 'src.celery_app.collect_crypto_data_task',
        'schedule': crontab(minute=0),  # Каждый час
        'args': (['BTC/USDT', 'ETH/USDT'], '1h'),
    },
    'collect-news-30min': {
        'task': 'src.celery_app.collect_news_task',
        'schedule': crontab(minute='*/30'),  # Каждые 30 минут
        'args': (['business', 'technology'], 'us'),
    },
    'analyze-market-daily': {
        'task': 'src.celery_app.analyze_market_task',
...
def collect_stock_data_task(tickers, period="1d", interval="1d"):
    ...
    from src.data_collectors import StockDataCollector
...
def collect_crypto_data_task(symbols, timeframe="1d"):
    ...
    from src.data_collectors import CryptoDataCollector
...
def collect_news_task(categories, country="us"):
    ...
    from src.data_collectors import NewsCollector
...
def analyze_sentiment_task(category="business", country="us"):
    ...
    from src.data_collectors import NewsCollector, SentimentAnalyzer
